const { test, expect } = require('@playwright/test');

const BASE_URL = 'https://www.swifttranslator.com/';

// 🔁 Open site before each test
test.beforeEach(async ({ page }) => {
  await page.goto(BASE_URL);
});

test('Pos_Fun_0001 - Health inquiry sentence', async ({ page }) => {
  const input = page.locator('textarea[placeholder="Input Your Singlish Text Here."]');
  await expect(input).toBeVisible();

  await input.fill('Oyaage saepa saniipa kohamadha?');
  await page.waitForTimeout(3000);

  const result = await page.locator('textarea').last().inputValue();
  console.log('Pos_Fun_0001 ->', result);
});

test('Pos_Fun_0002 - Long descriptive sentence', async ({ page }) => {
  const input = page.locator('textarea[placeholder="Input Your Singlish Text Here."]');
  await expect(input).toBeVisible();

  await input.fill('mama sunaQQgu vunee maargayata gasak kadaagena vaetii thibu nisaa');
  await page.waitForTimeout(3000);

  const result = await page.locator('textarea').last().inputValue();
  console.log('Pos_Fun_0002 ->', result);
});

test('Pos_Fun_0003 - Time & reason sentence', async ({ page }) => {
  const input = page.locator('textarea[placeholder="Input Your Singlish Text Here."]');
  await expect(input).toBeVisible();

  await input.fill('oyaata heta udhea 7.30 ta enna puLuvanne heathuva mata asaniipa nisaa');
  await page.waitForTimeout(3000);

  const result = await page.locator('textarea').last().inputValue();
  console.log('Pos_Fun_0003 ->', result);
});

test('Pos_Fun_0004 - Simple future sentence', async ({ page }) => {
  const input = page.locator('textarea[placeholder="Input Your Singlish Text Here."]');
  await expect(input).toBeVisible();

  await input.fill('Api heta enavaa');
  await page.waitForTimeout(3000);

  const result = await page.locator('textarea').last().inputValue();
  console.log('Pos_Fun_0004 ->', result);
});

test('Pos_Fun_0005 - Mixed English + Singlish', async ({ page }) => {
  const input = page.locator('textarea[placeholder="Input Your Singlish Text Here."]');
  await expect(input).toBeVisible();

  await input.fill('mata help ekak karanna puLuvandha? Can you help please?');
  await page.waitForTimeout(3000);

  const result = await page.locator('textarea').last().inputValue();
  console.log('Pos_Fun_0005 ->', result);
});

test('Pos_Fun_0006 - Weather related sentence', async ({ page }) => {
  const input = page.locator('textarea[placeholder="Input Your Singlish Text Here."]');
  await expect(input).toBeVisible();

  await input.fill('mama gedhara yanavaa, eth vahina nisaa dhaenma yannee naehae');
  await page.waitForTimeout(3000);

  const result = await page.locator('textarea').last().inputValue();
  console.log('Pos_Fun_0006 ->', result);
});

test('Pos_Fun_0007 - Help request', async ({ page }) => {
  const input = page.locator('textarea[placeholder="Input Your Singlish Text Here."]');
  await expect(input).toBeVisible();

  await input.fill('mata udhavuvak karanna puLuvandha?');
  await page.waitForTimeout(3000);

  const result = await page.locator('textarea').last().inputValue();
  console.log('Pos_Fun_0007 ->', result);
});


test('Pos_Fun_0008 - Short command', async ({ page }) => {
  const input = page.locator('textarea[placeholder="Input Your Singlish Text Here."]');
  await expect(input).toBeVisible();

  await input.fill('vahaama enna.');
  await page.waitForTimeout(3000);

  console.log('Pos_Fun_0008 ->', await page.locator('textarea').last().inputValue());
});

test('Pos_Fun_0009 - Negative statement', async ({ page }) => {
  const input = page.locator('textarea[placeholder="Input Your Singlish Text Here."]');
  await expect(input).toBeVisible();

  await input.fill('oyaa ehema karannee naehae.');
  await page.waitForTimeout(3000);

  console.log('Pos_Fun_0009 ->', await page.locator('textarea').last().inputValue());
});

test('Pos_Fun_0010 - Meeting request sentence', async ({ page }) => {
  const input = page.locator('textarea[placeholder="Input Your Singlish Text Here."]');
  await expect(input).toBeVisible();

  await input.fill(
    'machan mata heta meeting ekee Zoom link eka email ekak vidhihata evanna puLuvandha?'
  );
  await page.waitForTimeout(3000);

  console.log('Pos_Fun_0010 ->', await page.locator('textarea').last().inputValue());
});

test('Pos_Fun_0011 - Simple statement', async ({ page }) => {
  const input = page.locator('textarea[placeholder="Input Your Singlish Text Here."]');
  await expect(input).toBeVisible();

  await input.fill('mama ehema karanavaa.');
  await page.waitForTimeout(3000);

  console.log('Pos_Fun_0011 ->', await page.locator('textarea').last().inputValue());
});

test('Pos_Fun_0012 - Past tense sentence', async ({ page }) => {
  const input = page.locator('textarea[placeholder="Input Your Singlish Text Here."]');
  await expect(input).toBeVisible();

  await input.fill('mama iiyee gamea giyaa.');
  await page.waitForTimeout(3000);

  console.log('Pos_Fun_0012 ->', await page.locator('textarea').last().inputValue());
});

test('Pos_Fun_0013 - Emotional sentence', async ({ page }) => {
  const input = page.locator('textarea[placeholder="Input Your Singlish Text Here."]');
  await expect(input).toBeVisible();

  await input.fill('mata dhuka hithenavaa');
  await page.waitForTimeout(3000);

  console.log('Pos_Fun_0013 ->', await page.locator('textarea').last().inputValue());
});

test('Pos_Fun_0014 - Work related sentence', async ({ page }) => {
  const input = page.locator('textarea[placeholder="Input Your Singlish Text Here."]');
  await expect(input).toBeVisible();

  await input.fill('Api dhaen vaeda karanavaa');
  await page.waitForTimeout(3000);

  console.log('Pos_Fun_0014 ->', await page.locator('textarea').last().inputValue());
});

test('Pos_Fun_0015 - Very long mixed sentence', async ({ page }) => {
  const input = page.locator('textarea[placeholder="Input Your Singlish Text Here."]');
  await expect(input).toBeVisible();

  await input.fill(
    'machan adha thiyena meeting ekata Zoom link eka email ekak vidhihata mata evanna puluvandha? 3.00 pm venna kalin eka evuvanam hodhayi. Mama office yanna kalin eka balanna oonaa. Email ekak evanna baeri nam WhatsApp message ekak hari evanna. Thanks!'
  );
  await page.waitForTimeout(3000);

  console.log('Pos_Fun_0015 ->', await page.locator('textarea').last().inputValue());
});

test('Pos_Fun_0016 - Short confirmation', async ({ page }) => {
  const input = page.locator('textarea[placeholder="Input Your Singlish Text Here."]');
  await expect(input).toBeVisible();

  await input.fill('hari hari');
  await page.waitForTimeout(3000);

  console.log('Pos_Fun_0016 ->', await page.locator('textarea').last().inputValue());
});

test('Pos_Fun_0017 - Compliment sentence', async ({ page }) => {
  const input = page.locator('textarea[placeholder="Input Your Singlish Text Here."]');
  await expect(input).toBeVisible();

  await input.fill('wow oyaa harima lassanayi!');
  await page.waitForTimeout(3000);

  console.log('Pos_Fun_0017 ->', await page.locator('textarea').last().inputValue());
});

test('Pos_Fun_0018 - Food request', async ({ page }) => {
  const input = page.locator('textarea[placeholder="Input Your Singlish Text Here."]');
  await expect(input).toBeVisible();

  await input.fill('mata bath oonee');
  await page.waitForTimeout(3000);

  console.log('Pos_Fun_0018 ->', await page.locator('textarea').last().inputValue());
});

test('Pos_Fun_0019 - Help offer', async ({ page }) => {
  const input = page.locator('textarea[placeholder="Input Your Singlish Text Here."]');
  await expect(input).toBeVisible();

  await input.fill('mama obata sahaya vennadha?');
  await page.waitForTimeout(3000);

  console.log('Pos_Fun_0019 ->', await page.locator('textarea').last().inputValue());
});

test('Pos_Fun_0020 - Friendly question', async ({ page }) => {
  const input = page.locator('textarea[placeholder="Input Your Singlish Text Here."]');
  await expect(input).toBeVisible();

  await input.fill('oyaagee saepa saniipa kohomadha?');
  await page.waitForTimeout(3000);

  console.log('Pos_Fun_0020 ->', await page.locator('textarea').last().inputValue());
});

test('Pos_Fun_0021 - Health question', async ({ page }) => {
  const input = page.locator('textarea[placeholder="Input Your Singlish Text Here."]');
  await expect(input).toBeVisible();

  await input.fill('oyaagee saepa saniipa kohomadha?');
  await page.waitForTimeout(3000);

  console.log('Pos_Fun_0021 ->', await page.locator('textarea').last().inputValue());
});

test('Pos_Fun_0022 - Travel sentence', async ({ page }) => {
  const input = page.locator('textarea[placeholder="Input Your Singlish Text Here."]');
  await expect(input).toBeVisible();

  await input.fill('man pansal yanavaa');
  await page.waitForTimeout(3000);

  console.log('Pos_Fun_0022 ->', await page.locator('textarea').last().inputValue());
});

test('Pos_Fun_0023 - Joined sentence', async ({ page }) => {
  const input = page.locator('textarea[placeholder="Input Your Singlish Text Here."]');
  await expect(input).toBeVisible();

  await input.fill('mama gedhara yanavaa oyaa enavadha?');
  await page.waitForTimeout(3000);

  console.log('Pos_Fun_0023 ->', await page.locator('textarea').last().inputValue());
});

test('Pos_Fun_0024 - Meeting time', async ({ page }) => {
  const input = page.locator('textarea[placeholder="Input Your Singlish Text Here."]');
  await expect(input).toBeVisible();

  await input.fill('Zoom meeting eka adha 7.30 AM');
  await page.waitForTimeout(3000);

  console.log('Pos_Fun_0024 ->', await page.locator('textarea').last().inputValue());
});

// ===================== POSITIVE UI TEST =====================

test('Pos_UI_0001 - Invitation', async ({ page }) => {
  const input = page.locator('textarea[placeholder="Input Your Singlish Text Here."]');
  await expect(input).toBeVisible();

  await input.fill('mama gedhara innavaa, obata enna puluvandha?');
  await page.waitForTimeout(3000);

  console.log('Pos_UI_0001 ->', await page.locator('textarea').last().inputValue());
});
